...
public class Log4jUtil {
	...
	public static void initLog4jByProperty() {
		PropertyConfigurator.configure(Log4jUtil.class.getClassLoader().getResource("log4j.properties"));
	}
	
	public static void initLog4jByProperty(String logFilePath) throws IOException {
		if (logFilePath == null)
			throw new IOException("file not found: " + logFilePath);
...
//			return nopLogger;
		...
		if (st.length >= 3)
			MDC.put("location", st[2]); // 出错处的堆栈信息
		return logger;
	}
